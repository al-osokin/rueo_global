...
- Починили батчевый `reparse`: фронт снова ходит на `/admin/articles/{lang}/reparse`, ошибки обрабатываются, после запуска карточка и статистика обновляются.
- Добавлена кнопка «Переразобрать открытую» (`/admin/articles/{lang}/{art_id}/reparse`) — пересобирает только выбранную статью и показывает предупреждение, если парсинг вернул ошибку.
...
### План правок после ревью статей A–af

- **Пробелы и курсив**: в `parser_v3/normalization.py` перестаём склеивать подряд идущие текстовые узлы без вставки пробела, чтобы сочетания вида `пит`ать (_или_) испытывать` не слипались и корректно передавались в этап ревью.
- **Обработка ссылок**: нормализуем хвосты `=<target>` и `<…>` — переводим их в `reference`/ℹ, чтобы искомая фраза не содержала «резекционных» суффиксов вроде `rezista>`.
- **Инкапсуляция пояснений**: `_extract_translation_from_explanation` дополняем логикой переноса содержательного текста из пояснения в перевод (например, «(класс моллюсков) = …»), а курсива — в заметки.
- **Расширение суффиксов**: переписываем `_expand_suffix_appending` — добавляем перебор хвостов целиком, но ограничиваемся фразами, начинающимися с предлогов, и исключаем одиночные существительные, чтобы не появлялись «приставать адгезию».
- **Игнорирование служебных меток**: пополняем `_IGNORABLE_LABELS` системными пометами (`_мед._`, `_физ._` и др.), чтобы они уходили в ℹ, а не в текст поиска.
- **Ручные добивки**: добавляем в `resolved_translations` поддержку поля `manual_phrases`, чтобы ревьюер мог сохранить корректную фразу для индекса даже при сложном исходнике.
- **Проверка**: после правок прогоняем `review_translations.py` по диапазонам `offset=60–120` и `offset=188–210`, сверяемся с существующими заметками и чистим лишние `ru_requires_review`.

### Наблюдения по конкретным статьям (2025‑10‑22)

- `[abon|i]` (`art_id=66`): после правок разбивка первых блоков адекватна, `auto_candidates` теперь включают «абонировать» и корректные формы для `~ebl/a`.
- `[acefal/o/j]` (`art_id=118`): пояснение «(класс моллюсков = …)» уходит в ℹ, в поиске остаются только «безголовые | двустворчатые».
- `[acid/imun|a]` (`art_id=137`): хвост `rezista>` больше не попадает в перевод.
- `[abstin|i]` (`art_id=103`): блок `[~ant/o, ~ul/o]` теперь тянет пары напрямую из исходных точек с запятой (`человек, практикующий воздержание | трезвенник | непьющий`), искусственные запятые исчезли.
- `[adher|i]` (`art_id=191`): убрали префикс «мед» из перевода; помета уходит в ℹ, а в группе остаётся «образовывать спайки».
- `[adip/o/hist/o]` (`art_id=205`): поправили парсер ссылок с вложенными скобками; перевод теперь «жировая ткань», а `(= gras(o)histo)` уходит в reference.
- `[adstring|i]` (`art_id=253`): текст на новой строке «(воз)действие, вязать, стягивать;» больше не отрывается в иллюстрацию; основная группа раскрывается до «оказывать вяжущее воздействие | вязать | стягивать | оказывать вяжущее воздействие», а `[~ec/o]` даёт «вяжущее действие | вяжущее воздействие | вяжущий эффект».
