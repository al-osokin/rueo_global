...
- `[abstin|i]` (`art_id=103`): базовые значения распознаются корректно, однако блок `[~ant/o, ~ul/o]` всё ещё «склеивает» запятой «человека, практикующего воздержание» с «трезвенник, непьющий». Нужно научить `_split_translation_groups` восстанавливать суффиксы по точкам с запятой в исходнике. Пока статья отмечена `needs_review`.
